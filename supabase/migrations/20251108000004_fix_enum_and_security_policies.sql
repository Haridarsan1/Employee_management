-- Fix user_role enum and apply stricter security policies
-- Migration: 20251108000001_fix_enum_and_security_policies.sql

-- First, ensure 'owner' exists in user_role enum (safe / idempotent)
DO $$
BEGIN
  -- If the enum type doesn't exist, don't try to alter it (it will be created by the main schema migration)
...
EXCEPTION
  WHEN others THEN
    -- If ALTER TYPE fails for any reason, raise a clear error to surface it
    RAISE NOTICE 'Could not ensure owner in user_role enum: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;

-- Now apply stricter RLS policies and data integrity constraints
